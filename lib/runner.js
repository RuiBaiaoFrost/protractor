...
var loadDriver = function(config) {
  var driver;
...
var runTests = function() {
  ...
  var drivers = _.map(_.range(config.browsers), function() {
  ...
  drivers.forEach(function(driver, index) {
    promises.push(driver.getSession().then(function(session) {
      driver.manage().timeouts().setScriptTimeout(config.allScriptsTimeout);

      //TODO find out what will happen to saucelabs if I try to load more than 
      //one browser?
      if (drivers.length === 1) {
      ...
      //TODO what does this singleton exist for?
  ...
  var configurationComplete = Q.all(promises);  
  configurationComplete.done(function() {
    ...
    options.onComplete = function(runner, log) {
      ...
      var quitPromises = _.map(drivers, function(driver) {
        return driver.quit()
      });
      Q.all(quitPromises).done(function() {
